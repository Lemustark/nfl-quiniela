...
def register():
    if request.method == 'POST':
        ...
        if User.query.filter_by(username=username).first():
            flash(
                'El nombre de usuario ya está registrado. Elige otro o contacta al admin.',
                'danger',
            )
            return redirect(url_for('register'))
        ...
        if action == 'add_match':
            ...
            if not home_team or not away_team:
                ...
                if existing_matches:
                    ...
                    if not deadline_str:
                        flash(
                            'Al ser el primer partido de la semana, debes especificar la fecha y hora límite de cierre.',
                            'danger',
                        )
                ...
                db.session.commit()
                flash(
                    f'Encuentro añadido a la Semana {week}: {home_team} (Local) vs {away_team} (Visitante).',
                    'success',
                )
            ...
            if not new_username or not new_password:
                ...
                flash(f'El usuario "{new_username}" ya existe en el sistema.', 'danger')
            else:
                user_created = User(
            ...
            if user_to_toggle and user_to_toggle.username != 'admin':
                ...
                flash(f'Estatus de pago para "{user_to_toggle.username}" cambiado a: {estado}.', 'success')
            return redirect(url_for('admin', week=current_week) + '#users-section')
            ...
            if match:
                ...
                db.session.commit()
                flash(f'Encuentro {match.home_team} vs {match.away_team} reabierto con éxito.', 'success')
            return redirect(url_for('admin', week=current_week) + '#scores-section')
